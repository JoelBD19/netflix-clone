...
function Row({title, movies}: Props) {
    ...
    const handleClick = (direction: string) =>{
        ...
        if (rowRef.current) {
            ...
            const scrollTo = direction === "left" ? scrollLeft - clientWidth : scrollLeft
            + clientWidth
    ...
    return (
        <div className="h-40 space-y-0.5 md:space-y-2">
            ...
            <div className="group relative md:-ml-2">
                ...
                opacity-0 transition hover:scale-125 group-hover:opacity-100 ${!isMoved && "hidden"}`} onClick={() => handleClick("left")}/>

                <div ref={rowRef} className="flex scrollbar-hide items-center space-x-0.5 overflow-x-scroll md:space-x-2.5
                md:p-2">
                    {movies.map((movie) =>(
